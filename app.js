...
const TIKTOK_SETTINGS = {
    ...
    bitrateBase1080: 12e6,   // bitrate dasar utk 1080p @30fps (Maska riil: 12.71M)
    bitrateCap: 20e6,        // batas atas bitrate (60fps)
    bitrateFloor: 3e6,       // batas bawah (video kecil)
...
async function optimizeVideoForTikTok(file) {
    ...
    try {
        ...
        if (s.hybridEnabled && !already1080) {
            ...
            logMessage(
                `Hybrid (Editing News): downscale ${sourceWidth || "?"}x${sourceHeight || "?"} (fit, no pad) -> bitrate ${bitrateStr} (crf ${s.crf}), GOP ${gopSize} fr, fps asli ${srcFps || "?"}...`,
                "info",
            );
            ...
            if (ret === 0) {
                ...
                if (ret !== 0) {
                    ...
                    usedEncode = true;
                    logMessage(
                        `Encoding: keep ${sourceWidth}x${sourceHeight}, keep original fps, H.264 High crf${s.crf}/${bitrateStr}, BT.709...`,
                        "info",
                    );
                }
            }
