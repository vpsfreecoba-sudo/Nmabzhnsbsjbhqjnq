...
async function optimizeVideoForTikTok(file) {
    ...
    try {
        ...
        // ---- jalur 1: perlu downscale -> encode (HEVC, fallback H.264) ----
        if (s.hybridEnabled && !already1080) {
            logMessage(
                `Hybrid (Editing News): downscale ${sourceWidth || "?"}x${sourceHeight || "?"} -> ${codec === "libx265" ? "HEVC" : "H.264"} ${bitrateStr} (crf ${s.crf}), GOP ${gopSize} fr, fps asli ${srcFps || "?"}...`,
                "info",
            );
            ...
            // ---- jalur 2: sudah <=1080p -> stream copy (pertahankan codec asli) ----
            logMessage(
                `Fast mode: stream copy — ${sourceWidth || "?"}x${sourceHeight || "?"} (<=1080p), fps asli...`,
                "info",
            );
            ret = await instance.exec(copyArgs);
            if (ret !== 0) {
                logMessage("  Stream copy gagal — re-encode (HEVC, fallback H.264)...", "warning");
                ffmpegLog = "";
                let r = await tryEncode(filter);
                if (r.status === "ok") { usedEncode = true; ret = 0; }
                else if (r.status === "hang" || (r.status === "error" && codec === "libx265")) {
                    logMessage("  HEVC gagal/hang — fallback H.264...", "warning");
                    codec = "libx264"; codecTag = null;
                    codecProfile = s.profile; codecLevel = s.level;
                    await reloadInstance();
                    ffmpegLog = "";
        ...
        if (usedHybrid) {
            logMessage(
                `  OK: Hybrid — ${codec === "libx265" ? "HEVC (hvc1)" : "H.264 (avc1)"} ${bitrateStr}, fps asli, siap di-fuzz`,
                "success",
            );
        } else if (!usedEncode) {
            logMessage(
