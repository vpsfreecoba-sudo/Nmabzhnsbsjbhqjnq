...
async function optimizeVideoForTikTok(file) {
    ...
    try {
        ...
        // ============================================================
        //  CODEC ENCODING BROWSER: H.265 / HEVC (libx265) + TAG hvc1
        //  Sama seperti Hybrid Mode (Mode 1), dipadukan dengan -preset ultrafast
        //  dan parameter x265 ringan agar CPU HP dapat melakukan encoding HEVC
        //  dengan cepat & tidak macet.
        // ============================================================
        let codec = "libx265";
        let codecTag = "hvc1";
        let codecProfile = "main";
        let codecLevel = "5.1";
        ...
        function buildVArgs(vf) {
            ...
            if (codecTag) {
                a.push("-tag:v", codecTag);
            }
            a.push(
                "-profile:v", codecProfile,
                "-level", codecLevel,
                "-pix_fmt", s.pixFmt || "yuv420p",
                "-crf", String(s.crf || 17),
                "-maxrate", bitrateStr,
                "-bufsize", bufsizeStr,
                "-g", String(gopSize),
                "-color_primaries", s.colorPrimaries || "bt709",
                "-color_trc", s.colorTrc || "bt709",
                "-colorspace", s.colorSpace || "bt709",
            );
            if (codec === "libx265") {
                a.push("-x265-params", "log-level=error:no-open-gop=1:bframes=0:rc-lookahead=5");
            } else {
                a.push("-bf", String(s.bframes || 2), "-refs", String(s.refs || 2));
            }
            a.push(
                "-map_metadata", "-1",
                "-c:a", s.audioCodec || "aac",
                "-b:a", s.audioBitrate || "192k",
                "-ar", "48000",
                "-ac", "2",
                "-movflags", s.movflags || "+faststart",
                "-threads", String(s.threads || 4),
                outputName,
            );
            return a;
        }
        ...
        // ---- jalur 1: perlu downscale (>1080p) -> HEVC (hvc1) Hybrid ----
        if (s.hybridEnabled && !already1080) {
            logMessage(
                `Mode 1 (Hybrid HEVC): ${sourceWidth || "?"}x${sourceHeight || "?"} -> HEVC (hvc1) ${bitrateStr} (crf ${s.crf || 17}), GOP ${gopSize} fr, fps asli ${srcFps || "?"}...`,
                "info",
            );
            ...
            if (r.status === "ok") {
                ...
                logMessage("  HEVC menggantung/gagal — fallback H.264...", "warning");
                codec = "libx264"; codecTag = null; codecProfile = s.profile || "high"; codecLevel = s.level || "4.2";
                await reloadInstance();
                r = await tryEncode(scaleFilter);
                if (r.status === "ok") { usedHybrid = true; usedEncode = true; ret = 0; }
                else ret = await instance.exec(copyArgs);
            }
        } else {
            // ---- jalur 2: native <=1080p -> HEVC (hvc1) 1080p (seperti Hybrid Mode 1) ----
            logMessage(
                `Mode 2 (HEVC 1080p Hybrid): ${sourceWidth || "?"}x${sourceHeight || "?"} -> HEVC (hvc1) ${bitrateStr} (crf ${s.crf || 17}), GOP ${gopSize} fr...`,
                "info",
            );
            ...
            if (r.status === "ok") {
                ...
                if (r.status === "ok") { usedEncode = true; ret = 0; }
                    ...
                    logMessage("  Re-encode H.264 gagal — fallback Stream Copy...", "warning");
        ...
        if (usedHybrid) {
            logMessage(
                `  OK: Mode 1 (Hybrid HEVC) — ${codec === "libx265" ? "HEVC (hvc1)" : "H.264 (avc1)"} ${bitrateStr}, fps asli, siap di-fuzz`,
                "success",
            );
        } else if (usedEncode) {
            logMessage(
                `  OK: Mode 2 (HEVC 1080p Hybrid) — ${codec === "libx265" ? "HEVC (hvc1)" : "H.264 (avc1)"} ${bitrateStr}, fps asli, siap di-fuzz`,
                "success",
            );
        } else {
            logMessage(
                "  OK: Stream Copy — Resolusi & Kualitas Asli 100% Utuh (Siap Injeksi Fuzz)",
                "success",
            );
        }
