...
const TIKTOK_SETTINGS = {
    ...
    hybridBitrate1080: "12.5M", // bitrate utk 1080p (file Maska riil: 12.71 Mbps)
    keyFrameSeconds: 1,      // persis file Maska riil: GOP ±1.0 s
...
async function optimizeVideoForTikTok(file) {
    ...
    try {
        ...
        // Meniru Maska riil: 1080p -> 12.5 Mbps (file asli: 12.71 Mbps)
        if (area >= 1.8e6 && s.hybridBitrate1080) {
            const high = parseFloat(s.hybridBitrate1080) * 1e6;
            if (high > bitrateBps) bitrateBps = high;
        }
        const bitrateStr =
            bitrateBps >= 1e6
