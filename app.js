...
async function optimizeVideoForTikTok(file) {
    ...
    try {
        ...
        logMessage(
            `Fast mode: stream copy — keep ${sourceWidth || "?"}x${sourceHeight || "?"}, keep fps asli...`,
            "info",
        );
        showProgress();
        ...
        let ret = await instance.exec(copyArgs);
        if (ret !== 0) {
            logMessage(
...
patchBtn.addEventListener("click", async () => {
    ...
    logMessage("Fast mode: stream copy — resolusi & fps 100% asli", "info");
    logMessage("Container inject: fake samples (stsz/stts/stsc/stco)", "info");
