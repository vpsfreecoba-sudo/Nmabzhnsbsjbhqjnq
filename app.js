...
// ============================================================
//  FFMPEG ENCODER — jsDelivr CDN + toBlobURL + DETAILED ERROR LOGGING
// ============================================================
...
async function getFFmpeg() {
  ...
  try {
    ...
    // Gunakan jsDelivr CDN yang lebih stabil + toBlobURL untuk bypass CORS
    ...
    // Fetch core dan wasm sebagai blob URL untuk menghindari CORS issue
    ...
    if (err === undefined) {
      errorMessage = "Undefined error (possible network issue or CDN unreachable)";
    } else if (err === null) {
      ...
      if (err.stack) console.error("FFmpeg load stack:", err.stack);
    } else {
      errorMessage = String(err);
    }
    logMessage(`FFmpeg load error: ${errorMessage}`, "error");
...
async function encodeVideoWithFFmpeg(file, targetRes = 1080) {
  ...
  try {
    ...
    let errorMessage = "Unknown error";
    if (err === undefined) {
      errorMessage = "Undefined error (possible FFmpeg crash, out of memory, or worker killed)";
    } else if (err === null) {
      errorMessage = "Null error";
    } else if (typeof err === "string") {
      errorMessage = err;
      ...
      errorMessage = err.message || err.toString?.() || JSON.stringify(err);
      if (err.stack) console.error("Encode stack:", err.stack);
      if (err.cause) errorMessage += ` | Caused by: ${err.cause}`;
    } else {
      errorMessage = String(err);
    }

    logMessage(`Encode error: ${errorMessage}`, "error");
    throw new Error(`Encoding failed: ${errorMessage}`);
  }
}

// ============================================================
//  PATCH SINGLE FILE — DENGAN ERROR HANDLING YANG LEBIH BAIK
// ============================================================
async function patchSingleFile(item) {
  ...
  try {
    ...
    let errorMessage = "Unknown error";
    if (err === undefined) {
      errorMessage = "Undefined error (possible FFmpeg crash or unhandled rejection)";
    } else if (err === null) {
      errorMessage = "Null error";
    } else if (typeof err === "string") {
      errorMessage = err;
      ...
      errorMessage = err.message || err.toString?.() || JSON.stringify(err);
      if (err.stack) console.error("patchSingleFile stack:", err.stack);
      if (err.cause) errorMessage += ` | Caused by: ${err.cause}`;
    } else {
      errorMessage = String(err);
    }
    console.error("patchSingleFile error:", err);
    throw new Error(errorMessage);
  }
}

// ============================================================
//  EVENT HANDLER PATCH / DOWNLOAD — DENGAN ERROR LOGGING LEBIH DETAIL
// ============================================================
patchBtn.addEventListener("click", async () => {
  ...
  for (let i = 0; i < pendingItems.length; i++) {
    ...
    try {
      ...
      if (err === undefined) {
        errorMsg = "Undefined error (check console for details)";
      } else if (err === null) {
        errorMsg = "Null error";
      } else if (typeof err === "string") {
        errorMsg = err;
      } else if (err instanceof Error) {
        errorMsg = err.message || "Error without message";
      } else {
        errorMsg = String(err);
      }
