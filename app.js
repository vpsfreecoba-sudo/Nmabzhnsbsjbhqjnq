...
function updatePatchButton() {
    ...
    if (currentFlowState === "completed") {
        ...
        const settingsChanged = currentRes !== lastPatchedRes;

        if (settingsChanged) {
            patchBtn.disabled = false;
            patchBtn.querySelector("span").textContent = "Re-optimize";
        } else {
...
// ============================================================
//  VIDEO INFO DETECTION (via Video Element)
// ============================================================
    ...
    return new Promise((resolve) => {
        ...
        video.onloadedmetadata = () => {
            ...
            cleanup({
                ...
                fps: 30, // default, will be refined
            });
        };
        video.onerror = () => {
            cleanup(null);
...
// ============================================================
//  TIKTOK OPTIMIZE ENGINE 2026
// ============================================================
async function optimizeVideoForTikTok(file, targetRes = 1080) {
    ...
    try {
        ...
        if (videoInfo) {
            ...
            logMessage(
                `  Source: ${videoInfo.width}x${videoInfo.height} (${isLandscape ? "landscape" : "portrait"})`,
                "info",
            );
        }
        ...
        if (isLandscape) {
            // Landscape: scale by height, pad sides
            scaleFilter = `scale=-2:${targetRes}:flags=lanczos,pad=${targetRes}:${targetRes}:(ow-iw)/2:(oh-ih)/2:black`;
        } else {
            // Portrait: scale by width, pad top/bottom
            scaleFilter = `scale=${targetRes}:-2:flags=lanczos,pad=${targetRes}:${targetRes}:(ow-iw)/2:(oh-ih)/2:black`;
        }

        // FPS: interpolate ke 60fps kalo source < 55fps
        ...
        if (sourceFps < 55) {
            ...
            logMessage("  Frame rate: 60fps", "info");
        }

        // Color correction untuk TikTok (slight saturation + contrast boost)
        const colorFilter = `${fpsFilter},eq=saturation=1.05:contrast=1.02`;
        ...
        logMessage(
            `Encoding: ${targetRes}x${targetRes}, 60fps, H.264, 12Mbps...`,
            "info",
        );
        showProgress();
...
async function patchSingleFile(item) {
    ...
    // Optimize video dengan FFmpeg
    const result = await optimizeVideoForTikTok(item.file, targetRes);
    if (isCancelled) throw new Error("Cancelled");
