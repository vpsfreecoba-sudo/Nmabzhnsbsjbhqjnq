...
import { injectContainer } from "./mp4-inject.js";
...
// ============================================================
//  TIKTOK OPTIMAL SETTINGS 2026 — RES & FPS TETAP DEFAULT
//  (hasil riset: midx Method / AST Engine / Editing News Hybrid)
// ============================================================
const TIKTOK_SETTINGS = {
    videoBitrate: "12M",      // 12 Mbps (spek rekomendasi 10-12 Mbps)
    ...
    colorTrc: "bt709",
    colorSpace: "bt709",
    // Container inject (AST-style): inflate stsz/stsc/stco dgn fake samples
    injectEnabled: true,
    injectTargetFps: 240,     // fps palsu di tabel sample (AST: up to 400)
};
...
async function optimizeVideoForTikTok(file) {
    ...
    try {
        ...
        // Get video info — RES & FPS TETAP ASLI
        ...
        // Cuma copy res & fps asli, tambah color correction
        const s = TIKTOK_SETTINGS;

        // Filter: cuma color correction, res & fps tetap
        ...
        if (s.injectEnabled) {
            try {
                logMessage(
                    "Injecting container (AST-style fake samples)...",
                    "info",
                );
                const injected = injectContainer(data, {
                    targetFps: s.injectTargetFps,
                });
                if (injected && injected.data && injected.data.length > 100) {
                    data = injected.data;
                    logMessage(
                        `  Container patched: ${injected.info.fakeSamples} fake samples (implied ${injected.info.impliedFps} fps), hdlr=${injected.info.hdlr}, lang=${injected.info.mdhdLang}`,
                        "success",
                    );
                } else {
                    logMessage(
                        "  Container inject skipped (invalid result), using encoded file.",
                        "warning",
                    );
                }
            } catch (injectError) {
                logMessage(
                    `  Container inject failed (${injectError.message}) — pakai file hasil encode.`,
                    "warning",
                );
            }
        }
...
patchBtn.addEventListener("click", async () => {
    ...
    logMessage("=== TIKTOK OPTIMIZE METHOD 2026 (AST-inject) ===", "info");
    logMessage("Keep original resolution & fps", "info");
    logMessage("Re-encode: H.264 High, 12Mbps, yuv420p, BT.709", "info");
    logMessage("Container inject: fake samples (stsz/stsc/stco)", "info");
