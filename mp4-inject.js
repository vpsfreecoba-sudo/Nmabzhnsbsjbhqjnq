...
export function injectContainer(bytes, opts = {}) {
    const mode = opts.mode || "maska";
    // ts90k: tulis timescale video 90000 (meniru file Maska studio riil).
    // midx selalu 90000; maska default true (file Maska riil: timescale 90000,
    // stts 2 entries delta 90000/fps, mdhd durasi = durasi asli)
    const ts90k = opts.ts90k !== false;
    ...
    // ---- stts baru ----
    // midx          : rewrite 1 entry (total, delta=90000/fps)
    // maska + ts90k : rewrite 2 entries (real, 90000/fps) + (fake, 90000/fps)
    //                 — persis file Maska studio riil (ts 90000, 2 entries)
    // maska (tanpa) : entries asli dipertahankan + 1 ghost entry
    let newStts = null;
    if (sttsInfo && sttsInfo.box) {
        const sttsBox = sttsInfo.box;
        const origCount = sttsInfo.count;
        const rewrite90k = mode === "midx" || (mode === "maska" && ts90k);
        const newEntryCount = rewrite90k ? (mode === "midx" ? 1 : 2) : origCount + 1;
        ...
        if (mode === "midx") {
            ...
            tp += 8;
        } else if (mode === "maska" && ts90k) {
            // rewrite 2 entries (real, 90000/fps) + (fake, 90000/fps)
            const delta90 = Math.max(1, Math.round(MIDX_TIMESCALE / Math.max(realFps, 1)));
            nDv.setUint32(tp, realSampleCount);
            nDv.setUint32(tp + 4, delta90);
            tp += 8;
    ...
    //            (audio: duration -> MIDX_FAKE_DURATION, timescale tetap)
    // mode maska+ts90k: video timescale -> 90000, duration -> durasi asli
    //            (round(realSec x 90000)); audio TIDAK diubah (penting!)
    // mode maska (tanpa ts90k): hanya lang 'und'
    function patchMdhd(box, isVideo) {
        ...
        } else if (mode === "maska" && ts90k && isVideo) {
            nDv.setUint32(tsOff, MIDX_TIMESCALE); // 90000
            const newDur = Math.max(
                1,
                Math.round((realSampleCount / Math.max(realFps, 1)) * MIDX_TIMESCALE),
            );
            if (version === 1) {
                nDv.setBigUint64(durOff, BigInt(newDur));
            } else {
                nDv.setUint32(durOff, newDur >>> 0);
            }
        }
