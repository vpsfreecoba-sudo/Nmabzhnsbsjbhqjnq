// ============================================================
//  MP4 CONTAINER FUZZER — MASKA HYBRID (VERIFIED 1:1)
//  ============================================================
//  Direverse dari maska.dev/studio (worker mh1-CrGkOskX.js, fungsi Bt/Ut)
//  dan DIVERIFIKASI dengan menjalankan kode Maska asli + membandingkan
//  struktur box output-nya. Perilaku yang cocok:
//
//  1. stts  : entry asli TIDAK diubah; tambah 1 ghost entry
//             (fakeCount, deltaTerakhirAsli)
//  2. stsz  : tambah fakeCount entri ukuran 8 byte
//  3. stsc  : tambah 1 entry (realChunkCount+1, 1, desc)  -> 1 sample/chunk
//  4. stco  : realChunkCount + fakeCount entri;
//             semua fake chunk -> SATU offset dummy 8-byte di akhir mdat
//  5. mvhd & SEMUA tkhd : duration += floor(addTime x mvhdTimescale)
//             (Maska: addTime=0.3, mvhd timescale=1000 -> +300)
//  6. mdhd  : TIDAK diubah (audio & video)  <- kesalahan lama: mdhd x5.56
//             merusak audio track!
//  7. Audio track: hanya tkhd +durDelta; stts/stsz/stsc/stco TIDAK disentuh
//
//  Video & audio bitstream TIDAK di-re-encode (stream copy).
// ============================================================
...
export function injectContainer(bytes, opts = {}) {
    ...
    // ---- hitung fake count (Maska: percent=18 -> 5.56x) ----
    ...
    // Maska: 1 sample per fake chunk -> fakeChunkCount = fakeCount
    const fakeChunkCount = fakeCount;

    // ---- durDelta dari MVHD TIMESCALE (Maska: floor(addTime x timescale)) ----
    const mvhdInfo = getMvhd(data, moov);
    const durDelta = mvhdInfo
        ? Math.floor(addTime * mvhdInfo.timescale)
        : Math.floor(addTime * 1000);

    // ============================================================
    //  BANGUN BOX BARU (persis Maska)
    // ============================================================

    // ---- stts baru: entries asli TIDAK diubah + 1 ghost entry ----
    let newStts = null;
    if (sttsInfo && sttsInfo.box) {
        const sttsBox = sttsInfo.box;
        const origCount = sttsInfo.count;
        newStts = new Uint8Array(sttsBox.headerSize + 8 + (origCount + 1) * 8);
    ...
    // Geser offset SEMUA stco/co64 di track lain (audio dll) +delta.
    // (Maska & NoBlur melakukan ini — tanpa ini audio rusak karena
    //  offset-nya masih menunjuk posisi lama setelah moov membesar.)
    function buildOtherChunkPatches(deltaVal) {
        const patches = new Map();
        const allStco = findBoxes(data, moovContentStart, moov.size, "stco");
        const allCo64 = findBoxes(data, moovContentStart, moov.size, "co64");
        for (const box of [...allStco, ...allCo64]) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (is64) {
                    pDv.setBigUint64(rel, BigInt(readU64(dv, abs) + deltaVal), false);
                } else {
                    pDv.setUint32(rel, (readU32(dv, abs) + deltaVal) >>> 0, false);
                }
            }
    ...
    function buildReplaceMap(deltaVal) {
        const map = new Map();
        for (const [start, bytes] of fuzzPatches) {
            map.set(start, { end: start + bytes.length, bytes });
        }
        // offset audio & track lain digeser +delta
        for (const [start, entry] of buildOtherChunkPatches(deltaVal)) {
            map.set(start, entry);
        }
        ...
        for (const t of tracks) {
            ...
            // mdhd: hanya lang 'und' (durasi TIDAK diubah — penting utk audio!)
    ...
    // ---- isi offset chunk (PASS 2 pakai replaceMap yang sama) ----
    ...
    for (let j = 0; j < fakeChunkCount; j++) {
        // SEMUA fake chunk -> SATU offset dummy (persis Maska)
    ...
    // ---- rebuild moov FINAL (dengan audio offset digeser +delta) ----
    replaceMap = buildReplaceMap(delta);
